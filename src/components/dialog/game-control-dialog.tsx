...
import { AlertCircle, Users, Power, Clock, DollarSign, Activity, Search, Filter, Dice1Icon as Dice, RotateCcw } from 'lucide-react'
...
export function GameControlDialog({
  ...
  const [gameEndremains, setGameEndremains] = useState<string>("Calculating...")
  ...
  async function getUsersBet(skip: number = 0, take: number = 10) { 
    ...
    setUsersBets(resposne.data.getAllRecordsBySessionId.data as RecordSessionKqj[])
  }
  ...
  const onPageChange = (page: number) => {
    ...
    if (searchMode) onSearch((page - 1) * paginationState.pageSize, paginationState.pageSize);
    else getUsersBet((page - 1) * paginationState.pageSize, paginationState.pageSize);
  };
  ...
  const handleDrawResult = () => {
    ...
    if (drawMethod === "random") {
      drawnResult = Math.floor(Math.random() * (randomRange[1] - randomRange[0] + 1)) + randomRange[0]
  ...
  const handleRemoveUser = (userId: number) => {
    ...
    toast({
      ...
      description: "All bets of the user have been revoked. Tap Undo to restore.",
      action: (
        <ToastAction
  ...
  async function handleRemoveBet (sessionId: number, newAmount: number)  {
    ...
    console.log(response);
    if (response.status) {
      toastify.success(`User with userId ${sessionId} has been removed from this game`)
  ...
  function updateTime(updateState: boolean = true, timer: NodeJS.Timeout): string {
    ...
    setProgress(Math.max(0, (remainingTimeMs / (endTime.getTime() - new Date(game.session_start_time.toString()).getTime())) * 100));
  ...
  const filteredUsers = usersBets
    .filter(user => 
      ...
      user.user?.id.toString().toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      if (a.user === undefined || b.user === undefined) return 0
      if (sortBy === "betAmount") {
        const aBetAmountInt = TokenValues[a.token?.toString() as keyof typeof TokenValues]
        const bBetAmountInt = TokenValues[b.token?.toString() as keyof typeof TokenValues]
        return sortOrder === "asc" ? aBetAmountInt - bBetAmountInt : aBetAmountInt - bBetAmountInt
      } else {
