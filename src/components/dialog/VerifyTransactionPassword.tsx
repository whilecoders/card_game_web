...
import { Button, Drawer, Input, InputNumber, message, Modal, Tag } from "antd";
...
const VerifyTransactionPasswordModel = (params: TransactionPasswordType) => {
  const router = useRouter();
  const { selectedUserId, amount } = params;
  const [isTransactionPasswordExist, setIsTransactionPasswordExist] = useState(false)
  ...
  const verifyTransactionPassword = async (): Promise<boolean> => {
    ...
    if (!workerId) {
      toast.error("Something went wrong. Try re-login")
      return false;
    }
    ...
    if (!verifyPassRes.status) {
      if (verifyPassRes.message === 'Transaction password is not set') {
        setIsTransactionPasswordExist(true)
        params.openTransactinoPassword = false
      }
  ...
  async function checkIfUserHavePassSet() {
    //  fetching userid of login user from cookie
    const workerId = getCookie("id");
    if (!workerId) {
      toast.error("Something went wrong. Try re-login")
    ...
    setConfirmLoading(true)
    const getUserRes = await ApiCall({
      router: router,
      ...
      variables: { "id": Number(workerId) },
    })
    console.log(getUserRes);
    if (!getUserRes.status) {
      toast.error(getUserRes.message)
      setConfirmLoading(false)
      return false;
    }
    
    const thisUserData: string | undefined = getUserRes.data?.getUserById?.transaction_password  ?? undefined
    if (!thisUserData) {
      console.log("transaction password is not set");
      toast.info("You don't have transaction password.")
    }
    setIsTransactionPasswordExist(!!thisUserData)
    setConfirmLoading(false)
  }

  useEffect(() => {checkIfUserHavePassSet()}, [])

  return (
    <>
    {!isTransactionPasswordExist 
      ? <SetTransactionPassword 
        openTransactinoPassword={params.openTransactinoPassword}
        onChancel={(e) => params.onChancel?.(e)}
        onOk={e=>{
          checkIfUserHavePassSet()
          params.onChancel?.(e)
        }}
      /> 
      :<Modal
        title="Enter Transaction Passowrd"
        open={params.openTransactinoPassword}
  ...
  export default VerifyTransactionPasswordModel;