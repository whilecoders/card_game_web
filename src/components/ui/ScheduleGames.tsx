...
import { CalendarDays, Clock, User, Hash, Search, Trash2, RefreshCw, Edit3 } from 'lucide-react'
...
const ScheduledGames: React.FC = () => {
  ...
  const [newDateRange, setNewDateRange] = useState<NoUndefinedRangeValueType<Dayjs> | null>()
  ...
  const { data, error, isLoading,  } = useQuery({
    ...
    queryFn: async (): Promise<Games[]> =>  {
      userId.current = !isNaN(Number(getCookie(`id`))) ? Number(getCookie("id")) : 0
      return fetchScheduledGames()
    },
  });

  const filteredGames = games.filter((game) => {
    if (!searchTerm || isNaN(Number(searchTerm)) || searchTerm === "") return true
  ...
  return (
    <div className="p-5 pb-9">
      <h2 className="text-2xl font-semibold mb-4 text-primary">Scheduled Games</h2>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center w-full sm:w-auto">
          ...
          />
          <Button variant="outline" className='bg-white pl-0  rounded-l-none' size="icon">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading 
          ...
          : filteredGames.map((game: Games) => (
            <Card key={game.id} className="w-full bg-card overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-[1.02]">
              <CardHeader className="bg-primary/5 p-0 py-3">
                <CardTitle className="flex justify-between items-center text-lg">
                  <span className="text-primary font-semibold">Game #{game.id}</span>
                  <Badge variant="default" color="grey" className="font-normal">{game.game_in_day} sessions</Badge>
                </CardTitle>
              </CardHeader>
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">
                      {new Date(game.start_date).toLocaleDateString()} to {new Date(game.end_date).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Clock className="w-4 h-4 text-primary" />
                    <span className="text-sm">
                      {game.start_time} - {game.end_time} <span className="font-medium">({game.game_duration})</span>
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <User className="w-4 h-4 text-primary" />
                    <span className="text-sm">
                      Created by <span className="font-medium">{game.createdBy}</span> on {new Date(game.createdAt!).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
              ...
              <CardFooter className="flex justify-between bg-muted/50 gap-2">
                <Button variant="destructive" size="sm" onClick={() => {
                  setSelectedGame(games.find(searched => searched.id === game.id) || null)
                  setIsDeleteDialogOpen(true)
                }}>
                  <Trash2 className="w-4 h-4 mr-2" />
                  Delete
                </Button>
                <Button variant="outline" size="sm" onClick={() => handleReschedule(game.id)}>
      ...
      <Dialog open={isRescheduleDialogOpen} onOpenChange={setIsRescheduleDialogOpen}>
        <DialogContent className='bg-white'>
          ...
          <DialogFooter>
            <Button disabled={!newDateRange ? true : false} onClick={e=>confirmReschedule()}>Confirm</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
