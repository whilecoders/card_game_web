...
import { getGameBetsById,  getGameSessionByIdApi,  setGameSessionResultApi } from "@/lib/gameSessionKqjApis";
...
type PaneProps = {
  ...
  ratio: string,
  onSelecteGameChange?: (value: any, option?: DefaultOptionType | DefaultOptionType[] | undefined) => void
}
...
const columns: TableProps<GameResultDataType>["columns"] = [
  {
    ...
    render: (d) => { 
      return <img src={d} width={55} height={55} className="border-2 text-center border-[#a5a5a57f] rounded-md p-[2px] pb-0" />
    },
  },
  {
    title: "Card Name",
...
const items: MenuProps['items'] = [
  {
    icon: <><TfiAnnouncement className="mr-3"/></>,
...
async function fetchGameSessionOptions(params: {setOptions: Dispatch<any>, router: any}) {
  ...
  try {
    ...
    let fetchedTodaysSession: GameSessionKqj[] = response.data?.getGameSessionsByDateOrToday;
    const selectOptions = fetchedTodaysSession.map((session: GameSessionKqj) => {
      return {
        id: session.id,
        value: session.id,
        status: session.session_status,
        label: `${new Date(session.session_start_time ?? "--").toLocaleTimeString()}-${new Date(session.session_end_time ?? "--").toLocaleTimeString()}`,
      };
    })
...
async function refactoringGameResultData(
  ...
  const cardDataMap = new Map<string, { users: Set<number>; totalTokens: number }>();
  const totalAmountBetOnGame = gameBets.reduce((sum,accum) => {
    return sum + (TokenValues[accum.token as unknown as keyof typeof TokenValues] ?? 0);
  },0)
  ...
  gameBets.forEach((bet) => {
    ...
    const tokenValue = TokenValues[bet.token as unknown as keyof typeof TokenValues];

    // Initialize card data if not already present
    if (!cardDataMap.has(card)) {cardDataMap.set(card, { users: new Set(), totalTokens: 0 });}
  ...
  function calculatingProfitFromCard(key: GameResultCards) {
    ...
    if (!cardData && !letterData && !symbolData) return { 
      companyProfit: 0, userProfit: 0 
    }
  ...
  const finalGameResult: GameResultDataType[] = Object.values(GameResultCards).map<GameResultDataType>((cardKey, index) => {  
    ...
    if (!cardData || !cardDataMap.has(cardKey)) return {
...
export default function App() {
  ...
  const [currentGameSession, setCurrentGameSession] = useState<GameSessionKqj>()
  ...
  async function init() {
    ...
    await fetchGameSessionOptions({setOptions: setGameSessionOptions, router});
  ...
  async function fetchingTheGameData() {
    ...
    if (!selectedGameId) {
      const gameId = gameSessionOptions.find(option => option.status === "LIVE")?.id ?? gameSessionOptions[0]?.id;
    ...
    const thisGameBets: RecordSessionKqj[] = await getGameBetsById(selectedGameId, router);
    const userBetsMap = new Map<number, { totalToken: number; cards: Record<string, number > }>();
    thisGameBets.forEach(bet => {
      ...
      try {
        ...
        const userBetTokenParsed = TokenValues[bet.token as unknown as keyof typeof TokenValues];
        userData.cards[bet.choosen_card] = (userData.cards[bet.choosen_card] ?? 0) + userBetTokenParsed;
    ...
    const totalAmountBettedOnGame = Array.from(userBetsMap.values()).reduce((sum, userData) => sum + userData.totalToken, 0);
  ...
  useEffect(() => {init()}, [])
  useEffect(() => {fetchingTheGameData()}, [gameSessionOptions, selectedGameId]);
...
const PaneOne = (probs: PaneProps) => {
  const [gameResultCardData, setGameResultCardData] = useState<GameResultDataType[]>([]);
  const [gameSaveAmountRatio, setGameSaveAmountRatio] = useState<{[key: string]: number}>({})
  const [messageApi, contextHolder] = message.useMessage()
  ...
  const showPromiseConfirm = (cardKey: GameResultCards) => {
    confirm({
      ...
      content: `After confirming this action. Result of this game will be ${cardKey.replaceAll("_", " ")}`,
      onOk: async () => {
        await setGameSessionResultApi(probs?.selectGameId ?? 0, cardKey, router, messageApi)
        ...
        <Select
          ...
          optionRender={(option) => (
            <Space className="flex w-full justify-between">
              <span className="font-semibold w-[20%]" role="img" aria-label={option.data.label as string}>
                {option.data.id}
              </span>
              <span className=" w-[60%]">
                {option.data.label}
